...
function addProductToTable(user) {
  ...
  for (var i = 0; i < user.products.length; i++) {
    ...
    s +=
      `
			<tr>
        <td><input type="checkbox" class="productCheckbox" value="` +
      madon +
      `"></td> <!-- Checkbox for selection -->ss
				<td>` +
      (i + 1) +
      `</td>
...
function updateCartQuantity(masp, newQuantity) {
  var requestData = {
    madon: masp,
    soluong: newQuantity,
  };
